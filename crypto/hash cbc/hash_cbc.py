...
assert FLAG.startswith("MTACTF{") and FLAG.endswith("}")
class HashCBC:
    def __init__(self):
    ...
    def encrypt(self, message):
        ...
        cipher = []
        for m in message:
            IV = self.make_iv(IV , self.key) ;
            c = hashlib.md5((IV + m).encode()).hexdigest()
            cipher.append(c)
        return cipher
