...
#
# MD21.py: An implementation of the MD21 hash algorithm in pure Python 3.
#
...
class MD21:
    """An implementation of the MD21 hash algorithm from MD4."""

    width = 32
    mask = 0xFFFFFFFF

    # Unlike, say, SHA-1, MD21 uses little-endian. Fascinating!
        ...
        for chunk in chunks:
            ...
            for round in range(21):
                ...
                for n in range(16):
                    ...
                    K, S = n, Xi[n % 4]
                    hn = h[i] + MD21.F(h[j], h[k], h[l]) + X[K]
                    h[i] = MD21.lrot(hn & MD21.mask, S)
                ...
                for n in range(16):
                    ...
                    hn = h[i] + MD21.G(h[j], h[k], h[l]) + X[K] + 0x5A827999
                    h[i] = MD21.lrot(hn & MD21.mask, S)
                ...
                for n in range(16):
                    i, j, k, l = map(lambda x: x % 4, range(-n, -n + 4))
                    K, S = Ki[n], Xi[n % 4]
                    hn = h[i] + MD21.H(h[j], h[k], h[l]) + X[K] + 0x6ED9EBA1
                    h[i] = MD21.lrot(hn & MD21.mask, S)

            self.h = [((v + n) & MD21.mask) for v, n in zip(self.h, h)]
    ...
    @staticmethod
    def lrot(value, n):
        lbits, rbits = (value << n) & MD21.mask, value >> (MD21.width - n)
        return lbits | rbits
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("Testing the MD21 class.")
        print()

        for message, expected in zip(messages, known_hashes):
            print("Message: ", message)
            print("Expected:", expected)
            print("Actual:  ", MD21(message).hexdigest())
            print()
